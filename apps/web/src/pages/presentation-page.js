...
export function PresentationPage() {
    ...
    return (_jsxs("div", { className: "space-y-4 p-6", children: [_jsxs("div", { className: "flex flex-wrap items-center gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-panel", children: [_jsx("select", { className: "rounded-[10px] border border-slate-200 px-3 py-2", onChange: (event) => setSearchParams({ periodId: event.target.value, compareToPeriodId: compareToPeriodId ?? "" }), value: periodId, children: (periodsQuery.data ?? []).map((period) => (_jsx("option", { value: period.id, children: period.title }, period.id))) }), _jsxs("select", { className: "rounded-[10px] border border-slate-200 px-3 py-2", onChange: (event) => setSearchParams({ periodId: periodId ?? "", compareToPeriodId: event.target.value }), value: compareToPeriodId ?? "", children: [_jsx("option", { value: "", children: "Karsilastirma yok" }), (periodsQuery.data ?? []).map((period) => (_jsx("option", { value: period.id, children: period.title }, period.id)))] }), _jsxs("div", { className: "ml-auto flex gap-2", children: [_jsx("button", { className: "rounded-full border px-4 py-2 text-sm font-semibold", onClick: () => setActiveSlide((current) => Math.max(0, current - 1)), type: "button", children: "Onceki" }), _jsx("button", { className: "rounded-full bg-brand-ink px-4 py-2 text-sm font-semibold text-white", onClick: () => setActiveSlide((current) => Math.min(slides.length - 1, current + 1)), type: "button", children: "Sonraki" })] })] }), visibleSlide] }));
}
