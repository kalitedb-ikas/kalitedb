...
export function SalesAuditPage() {
  ...
  // Sadece satış dönemlerini göster
  ...
  const sortedPeriods = useMemo(
    () => [...salesPeriods].sort((left, right) => left.month.localeCompare(right.month)),
    [salesPeriods]
  );

  const periodId = searchParams.get("periodId") ?? selectDefaultReportPeriod(salesPeriods)?.id;
  const selectedPeriod = salesPeriods.find((period) => period.id === periodId);
  ...
  const agents = useMemo<AuditAgentRow[]>(() => {
    ...
    const sortedAgents = Array.from(agentKeys)
      ...
      .sort((left, right) => {
        ...
        if (leftScore === null && rightScore === null) return left.agentName.localeCompare(right.agentName, "tr");
    ...
    return sortedAgents.map((agent, index) => ({ ...agent, listIndex: index + 1 }));
  ...
  const lowestAuditGroup = useMemo(() => {
    ...
    const leaders = scoredAgents
      ...
      .sort((left, right) => left.agentName.localeCompare(right.agentName, "tr"));
    return { names: leaders.map((agent) => agent.agentName).join(", "), score: lowestScore };
  }, [currentAudits]);
  ...
  const auditHistoryQueries = useQueries({
    queries: sortedPeriods.map((period) => ({
      ...
      queryFn: () => api.getDashboard(auth.token, period.id, undefined, { datasetTypes: ["audit-metrics"] }),
      staleTime: 5 * 60 * 1000
    }))
  });
  ...
  const auditHistory = useMemo(() => {
    ...
    return selectedYear
      ? buildYearTrendPoints({ year: selectedYear, currentPeriodId: periodId, points })
      : [];
  ...
  const performanceShift = useMemo(() => {
    if (!previousAuditDetail) return null;
    ...
    const changes = currentAudits
      ...
      .map((agent) => {
        ...
        return { label: agent.agentName, delta: Number((agent.auditScore - previousScore).toFixed(2)) };
      })
    ...
    if (highestIncrease > 0) {
      ...
      return { title: "Yükselen performans", names: leaders.map((item) => item.label).join(", "), delta: highestIncrease };
    }
    ...
    if (biggestDecrease < 0) {
      const leaders = changes
        ...
        .sort((left, right) => left.label.localeCompare(right.label, "tr"));
      return { title: "Düşen performans", names: leaders.map((item) => item.label).join(", "), delta: biggestDecrease };
    }
    return {
      title: "Aylık değişim",
      names: changes.map((item) => item.label).sort((left, right) => left.localeCompare(right, "tr")).join(", "),
      delta: 0
    };
  ...
  // Satış dönemleri henüz yoksa boş durum göster
  if (!periodId && !dashboardQuery.isFetching) {
    return (
      <div className="space-y-6">
        <PageHeader title="Satış Audit" />
        <SurfaceCard title="Henüz veri yok" variant="default">
          <p className="text-sm text-slate-600">
            Satış departmanına ait dönem henüz oluşturulmamış. Admin panelinden yeni bir dönem ekleyebilirsiniz.
          </p>
        </SurfaceCard>
      </div>
    );
        ...
        <>
          ...
          <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="grid gap-4">
              ...
              />
              <SurfaceCard title="Kısa özet" variant="default">
                <div className="space-y-3">
                  <SummaryLine title="Değerlendirilen temsilci" value={formatNumber(evaluatedAgentCount)} />
                  <SummaryLine title="En yüksek audit" value={formatAuditScore(snapshot.highlights.bestAudit?.value)} />
                  <SummaryLine title="En düşük audit" value={formatAuditScore(snapshot.highlights.lowestAudit?.value)} />
                </div>
              </SurfaceCard>
            </div>
          </div>
...
function SummaryLine(props: { title: string; value: string }) {
  return (
...
function buildAgentAvatar(name: string, index: number) {
  ...
  if (repositoryPhoto) return repositoryPhoto;
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 160 160">
      ...
      <text x="50%" y="54%" dominant-baseline="middle" text-anchor="middle" fill="white"
        font-family="Inter, Arial, sans-serif" font-size="52" font-weight="700" letter-spacing="4">
        ${initials}
      </text>
    </svg>
  `;
