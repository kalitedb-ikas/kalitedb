...
export function QuestionsPage() {
    ...
    return (_jsxs("div", { className: "space-y-6", children: [_jsx(PeriodFilters, { compareToPeriodId: compareToPeriodId, onCompareChange: (value) => setCompareToPeriodId(value || undefined), onPeriodChange: (value) => setPeriodId(value), periodId: periodId, periods: periodsQuery.data ?? [] }), _jsx(SectionCard, { title: "Konu filtresi", actions: _jsxs("select", { className: "rounded-[10px] border border-slate-200 px-3 py-2 text-sm", onChange: (event) => setTopic(event.target.value), value: topic, children: [_jsx("option", { value: "", children: "Tum konular" }), topics.map((item) => (_jsx("option", { value: item, children: item }, item)))] }), children: _jsxs("div", { className: "grid gap-4 xl:grid-cols-2", children: [_jsx(Leaderboard, { items: (dashboardQuery.data?.rankings.weakestQuestions ?? []).map((item) => ({
                                id: item.id,
