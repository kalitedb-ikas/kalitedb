                ...
                <tr key={row.agentKey} className="transition hover:bg-slate-50/60 dark:hover:bg-slate-700/30">
                  ...
                  {row.months.map((value, monthIdx) => (
                    <td key={monthIdx} className="px-2 py-2.5 text-center tabular-nums">
                      {value !== null ? (
                        <span className="inline-flex min-w-[36px] justify-center rounded-[10px] bg-slate-100 px-2 py-0.5 text-sm font-medium text-slate-700 dark:bg-slate-700/50 dark:text-slate-300">
                          {formatter(value)}
                        </span>
                      ) : (
